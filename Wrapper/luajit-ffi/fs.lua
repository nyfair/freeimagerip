...
function dir(pattern)
	...
	repeat
		...
		if fn ~= "." and fn ~= ".."
		tFiles[fn] = {
			attrib = fd.dwFileWttributes,
			time_create = fd.ftCreationTime,
			time_access = fd.ftLastAccessTime,
			time_write = fd.ftLastWriteTime,
...
function splitpath(fn)
	local idx = fn:match(".+()[/\].+$")
	if(idx) then
		return fn:sub(1, idx), fn:sub(idx + 1)
	else
		return "./", fn
	end
end
