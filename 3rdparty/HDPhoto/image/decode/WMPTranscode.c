...
//    if(pSCDec->WMISCP.cfColorFormat == YUV_422 && oO >= O_RCW && pParam->cfColorFormat != Y_ONLY)
    if(pSCDec->WMISCP.cfColorFormat == YUV_422 && oO >= O_RCW)
        pParam->oOrientation = oO = O_NONE; // Can not rotate 422 in compressed domain!

//    if(pParam->cfColorFormat != Y_ONLY) // xxx => Y_ONLY
//        if(pParam->cfColorFormat != YUV_444 || (pSCDec->WMISCP.cfColorFormat != CMYK && pSCDec->WMISCP.cfColorFormat != BAYER)) // CMYK/BAYER => YUV_444
//            pParam->cfColorFormat = pSCDec->WMISCP.cfColorFormat;
    
//    assert(pParam->uAlphaMode == 0);        // others not implemented yet!
